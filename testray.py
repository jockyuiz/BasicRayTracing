...
class TestSphereIntersect(unittest.TestCase):
    ...
    def test_unitsphere_hits(self):
        ...
        hit = self.confirm_hit(unit_sphere, Ray(vec([2.0,3.0,5.0]), vec([-2.0,-3.0,-4.0])))
        self.assertAlmostEqual(hit.t, 1.0)
        # dead center hit, start inside
        hit = self.confirm_hit(unit_sphere, Ray(vec([2.0,0.0,0.0]), vec([-1.0,0.0,0.0]), 1.1))
    ...
    def test_unitsphere_misses(self):
        ...
        # off axis miss
        hit = unit_sphere.intersect(Ray(vec([2.0,3.0,4.0]), vec([-1.0,-1.0,-1.0])))
        self.assertEqual(hit.t, np.inf)
        # dead center miss, start past sphere
        hit = unit_sphere.intersect(Ray(vec([2.0,0.0,0.0]), vec([-1.0,0.0,0.0]), 3.1))
        self.assertEqual(hit.t, np.inf)
    ...
    def test_nonunit_hits(self):
        ...
        self.assertAlmostEqual(hit.t, 1 - np.sin(np.pi/3))
        hit = self.confirm_hit(sphere, Ray(vec([5.0,4.0,5.0]), vec([-2.0,-3.0,-4.0])))
        self.assertAlmostEqual(hit.t, 3 - 3 / np.sqrt(29))
        hit = self.confirm_hit(sphere, Ray(vec([5.0,4.0,8.0]), vec([-2.0,-3.0,-4.0])))
        self.assertAlmostEqual(hit.t, 3.0)
...
class TestPoinLight(unittest.TestCase):
    ...
    def test_diffuse(self):
        ...
        np.testing.assert_allclose(
            self.shading_test(
                vec([1,2,3]), vec([0,0,1]), vec([1, 1, 1]),  # p, n, v
                vec([0,1,1]), 2, vec([0.6,0.3,0.2]),  # l, r, I
                Material(vec([0.2,0.4,0.6])), Scene([])
            ),
...
class TestTriangleIntersect(unittest.TestCase):
    ...
    def test_transformed(self):
        ...
        hit = tri.intersect(Ray(u + M @ [-0.3, 0.3, 1], M @ [0, 0, -1]))
        self.assertEqual(hit.t, np.inf)
        hit = tri.intersect(Ray(u + M @ [0.5, 0.55, 1], M @ [0, 0, -1]))
        self.assertEqual(hit.t, np.inf)
        hit = tri.intersect(Ray(u + M @ [0.2, -0.1, 1], M @ [0, 0, -1]))
        self.assertEqual(hit.t, np.inf)
